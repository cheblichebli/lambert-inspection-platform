...
router.get('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.error('Schedule GET single error:', err);
...
// Resets a schedule back to pending when inspector cancels
  ...
  try {
    ...
    const result = await db.query(`
      UPDATE inspection_schedules SET
        ...
        form_template_id = COALESCE($2, form_template_id),
      ...
      [title || null, form_template_id || null, assigned_to || null, frequency || null,
        start_date || null, end_date || null, location || null, equipment_id || null,
        notes || null, is_active !== undefined ? is_active : null, status || null, req.params.id]);
...
router.delete('/:id', authenticateToken, authorizeRoles('admin', 'supervisor'), async (req, res) => {
  ...
  try {
    ...
    if (!existing.rows.length) return res.status(404).json({ error: 'Schedule not found' });
    await db.query(
      'UPDATE inspection_schedules SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
      [req.params.id]
    );
