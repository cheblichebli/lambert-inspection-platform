...
// POST /api/schedules — create schedule (supervisor + admin only)
  ...
  try {
    const result = await db.query(`
      INSERT INTO inspection_schedules
        (title, form_template_id, assigned_to, frequency, start_date, end_date,
         location, equipment_id, notes, created_by)
...
// PUT /api/schedules/:id — update schedule (supervisor + admin only)
...
// DELETE /api/schedules/:id — soft delete (supervisor + admin only)
